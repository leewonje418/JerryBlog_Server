...
import LoginSuccessHandler from '../lib/handler/httpLoginSuccessHandler';
...
export default class AuthController {
    ...
    login = async (req: Request, res:Response) => {
        try {
            ...
            LoginSuccessHandler(response, 200, '로그인 성공', token);
        } catch (err) {
    ...
    logout = async (res: Response) => {
        SuccessHandler(response, 200, '로그아웃 성공');
    }
    ...
    user = async (req: any, res: Response) => {
        try {
            ...
            SuccessHandler(response, 200, '유저 전체조회 성공', user);
        } catch (err) {
            ErrorHandler(res, err);
        }
    }
